...
This document describes the on-disk storage layout used by Horizon when operating in **local box** mode. It serves as the authoritative reference for how key-value data is persisted, indexed, and managed at the filesystem level.
...
**API mapping:** This protocol backs the `Horizon_box_get_key_by_index` and `Horizon_box_get_key_size_by_index` functions documented in the [API Usage](api_usage.md) reference.
...
The API supports **chunked writes** (`Horizon_box_write_data_chunk`) and **offset-based reads** (`Horizon_box_read_data`). These operations map directly to file I/O on the `data` file:

| API Operation                             | Storage-Layer Action                                                     |
|-------------------------------------------|--------------------------------------------------------------------------|
| `Horizon_box_write_data`             | Overwrites the entire `database/itens/<keyhash>/data` file.              |
| `Horizon_box_write_data_chunk`       | Seeks to `offset` in `data` and writes `chunk_size` bytes.              |
| `Horizon_box_read_data`              | Seeks to `offset` in `data` and reads up to `size_to_read` bytes.       |
| `Horizon_box_get_data_size`          | Returns the file size of `database/itens/<keyhash>/data`.                |
...
| Scenario                                  | Behavior                                                                                    |
|-------------------------------------------|---------------------------------------------------------------------------------------------|
| Key does not exist (read/delete)          | The `<keyhash>` directory is absent. Returns an error via `Horizon_box_is_error`.       |
...
Since the `<keyhash>` is a one-way transformation, the original key bytes must be preserved to support key retrieval during iteration (`Horizon_box_get_key_by_index`).
